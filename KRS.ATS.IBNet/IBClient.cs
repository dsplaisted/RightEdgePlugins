...
namespace Krs.Ats.IBNet
{
    ...
    public class IBClient : IDisposable
    {
		...
		private static readonly ILog log = LogManager.GetLogger(typeof(IBClient));
                                      ...
                                      BarSize barSizeSetting, HistoricalDataType whatToShow, int useRth)
        {
            ...
            lock (this)
            {
                ...
                try
                {
					...
					//	This is the algorithm used by RightEdge before Krs implemented something similar (which commented out below)
					if (duration.TotalDays > 5)
					{
						...
						if (totalDays % 7 > 0)
							weeks++;
						send(weeks.ToString() + " W");
					}
					else if (duration.TotalDays > 1)
					{
						int totalDays = Convert.ToInt32(duration.TotalDays);
						if (duration.TotalDays > totalDays)
						{
							totalDays++;
						}
						send(totalDays.ToString() + " D");
					}
					else
					{
						int totalSeconds = Convert.ToInt32(duration.TotalSeconds);
						send(totalSeconds.ToString() + " S");
					}

					//	Below is the Krs algorithm.
        ...
        public void PlaceOrder(int orderId, Contract contract, Order order)
        {
            ...
            lock (this)
            {
                ...
                try
                {
					...
					send(string.Format(CultureInfo.InvariantCulture, "{0:F8}", order.LimitPrice));
					send(string.Format(CultureInfo.InvariantCulture, "{0:F8}", order.AuxPrice));
        ...
        private bool ProcessMsg(IncomingMessage msgId)
        {
			...
			try
			{
				log.DebugFormat("Processing message [{0}]", msgId.ToString());
				switch (msgId)
				{
					...
					case IncomingMessage.PortfolioValue:
						{
							...
							decimal marketPrice;
							//TryReadDecimal("PortfolioValue.MarketPrice", out marketPrice);
							marketPrice = ReadDecimal("PortfolioValue.MarketPrice");
							decimal marketValue = ReadDecimal("PortfolioValue.MarketValue");
							decimal averageCost = 0.0m;
							decimal unrealizedPNL = 0.0m;
							decimal realizedPNL = 0.0m;
							if (version >= 3)
							{
								averageCost = ReadDecimal("PortfolioValue.AverageCost");
								unrealizedPNL = ReadDecimal("PortfolioValue.UnrealizedPNL");
								realizedPNL = ReadDecimal("PortfolioValue.RealizedPNL");
							}
					...
					case IncomingMessage.OpenOrder:
						{
							...
							if (version >= 9)
							{
								...
								//	Old code (which had default as AgentDescription.Individual instead of None
								//string rule80 = ReadStr();
								//if (rule80 == null)
								//{
								//	order.Rule80A = AgentDescription.Individual;
								//}
								//else
								//{
								//	order.Rule80A =
								//		(AgentDescription)EnumDescConverter.GetEnumValue(typeof(AgentDescription), rule80);
								//}
					...
					case IncomingMessage.HistoricalData:
						{
							...
							//historicalData(reqId, DateTime.MinValue, -1, -1, -1, -1, -1, -1, -1, false, -1, -1);
							break;
						}
			...
			catch (Exception ex)
			{
				...
				throw new InvalidOperationException("Exception thrown while processing message: " + msgId.ToString(), ex);
			}
            return true;
        }
        ...
        private bool ReadBoolFromInt()
        {
			...
			{
				string msg = "Invalid bool value: " + str;
				log.Error(msg);
        ...
        private int ReadInt()
        {
			...
			{
				string msg = "Invalid int value: " + str;
				log.Error(msg);
				throw new InvalidDataException(msg, ex);
			}
        }
        ...
        private int ReadIntMax()
        {
			...
			{
				string msg = "Invalid decimal value: " + str;
				log.Error(msg);
				throw new InvalidDataException(msg, ex);
			}
        }
        ...
        private long ReadLong()
        {
			...
			{
				string msg = "Invalid long value: " + str;
				log.Error(msg);
				throw new InvalidDataException(msg, ex);
			}
        }
        ...
        private double ReadDouble()
        {
			...
			{
				string msg = "Invalid double value: " + str;
				log.Error(msg);
				throw new InvalidDataException(msg, ex);
			}
        }

		//private bool TryReadDecimal(string desc, out decimal value)
		//{
		//    String str = ReadStr();
		...
		//        return true;
		//    }
		//    string msg = "Invalid decimal value while reading " + desc + ": " + str;
		//    log.Warn(msg);
		...
		{
			return ReadDecimal(null);
		}
        private decimal ReadDecimal(string desc)
        {
            String str = ReadStr();
			try
			{
				return str == null ? 0 : Decimal.Parse(str, NumberStyles.AllowExponent | NumberStyles.Number,CultureInfo.InvariantCulture);
			}
			catch (Exception ex)
			{
